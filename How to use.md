# HJ Incremental XAMPP Backup — Complete How-To Guide
...
|---------|------|---------|
| **HJ Inc Backup** | `HJ_Inc_Backup_source/HJ Inc Backup/` | WPF Desktop GUI — manual backups, restores, service management |
| **HJ_Inc_Backup.Core** | `HJ_Inc_Backup_source/HJ_Inc_Backup.Core/` | Shared library — backup/restore engines, VSS, service controllers |
| **HJ_Inc_Backup.Service** | `HJ_Inc_Backup_source/HJ_Inc_Backup.Service/` | Windows Service — automated scheduled backups |
...
### 1. Launching
- Build & run `HJ Inc Backup` WPF project (requires **Administrator rights** — manifest enforces `requireAdministrator`)
...
**Option B** — Manually:
1. Publish the `HJ_Inc_Backup.Service` project
2. Run: `sc.exe create "HJ XAMPP Incremental Backup" binPath= "C:\path\to\published\HJ_Inc_Backup.Service.exe" start= auto`
...
| **Windows Service** | Windows Event Log → Application → source "HJ XAMPP Incremental Backup" |
